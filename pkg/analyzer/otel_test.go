...
func TestOTelSpanGeneration(t *testing.T) {
	...
	t.Run("Review markers emit correct OTel spans", func(t *testing.T) {
		...
		_, err := buildURLResult(ctx, parsed, 0, "sha", "main", "PR 1", "url", reviewEvents, nil, nil, 0, 0, nil, nil, nil)
	...
	t.Run("Commit markers are emitted when commitTimeMs is present", func(t *testing.T) {
		...
		_, err := buildURLResult(ctx, parsed, 0, "sha123", "main", "Commit sha123", "url", nil, nil, &commitTimeMs, 0, 0, nil, nil, nil)
		assert.NoError(t, err)
