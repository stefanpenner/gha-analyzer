...
type Model struct {
	...
	openPerfettoFunc func([]trace.ReadOnlySpan)
	// Mouse mode state
	mouseEnabled bool
...
// It receives the currently visible (non-hidden) spans.
type OpenPerfettoFunc func([]trace.ReadOnlySpan)
		...
		if m.showDetailModal {
			...
			case "p":
				if m.openPerfettoFunc != nil {
					m.openPerfettoFunc(m.visibleSpans())
				}
				return m, nil
			case "g":
				if m.inspectorFocusLeft {
